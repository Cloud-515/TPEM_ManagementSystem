...
export function simulatedEnergyTrend(range) { const snapshot = meters(); const count = range === '24h' ? 24 : range === '7d' ? 7 : 30; const today = new Date(); return { data: Array.from({ length: count }, (_, index) => { const date = new Date(today.getFullYear(), today.getMonth(), today.getDate() - (count - index - 1)); const label = range === '24h' ? `${pad(index)}:00` : `${pad(date.getMonth() + 1)}-${pad(date.getDate())}`; return { label, consumptionKwh: round(snapshot.reduce((sum, item) => sum + item.activePowerKw, 0) * (range === '24h' ? 1 : 24) * (0.86 + index / 500)), meterCount: snapshot.length } }) } }
