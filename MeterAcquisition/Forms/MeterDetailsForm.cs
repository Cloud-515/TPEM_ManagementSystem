...
namespace MeterAcquisition
{
    public class MeterDetailsForm : Form
    {
        ...
        public void RefreshFromMeter()
        {
            ...
            _lblStatus.ForeColor = status.Color;
        ...
        private (string Text, Color Color, bool IsOffline) GetDisplayStatus(MeterInfo meter)
        {
            if (Owner is MainForm mainForm)
            {
                return mainForm.GetMeterDisplayStatus(meter);
            }

            if (meter?.RealTime != null)
            {
                var status = MainForm.DetermineMeterStatus(meter.RealTime);
                return (status.text, status.color, false);
            }
